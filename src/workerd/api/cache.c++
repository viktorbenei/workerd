...
jsg::Promise<void> Cache::put(jsg::Lock& js, Request::Info requestOrUrl,
  ...
  return js.evalNow([&] {
    ...
    return startStreamPromise.then(js, context.addFunctor(
      ...
      // `co_magic BEGIN_DEFERRED_PROXYING` from our coroutine. Everything after that `co_magic`
      // constitutes the second async flow that actually handles the request and response.
      //
      ...
      // with our magic `co_magic`, then perform all our additional work. Then we
