...
// following features:
//
// - `co_magic BEGIN_DEFERRED_PROXYING` fulfills the outer kj::Promise<DeferredProxy<T>>. The
...
//   already been fulfilled by the magic `co_magic` described above), then fulfills the inner
//   `proxyTask`.
//
// - Unhandled exceptions reject the outer kj::Promise<DeferredProxy<T>> (if it has not already
//   been fulfilled by the magic `co_magic` described above), then reject the inner `proxyTask`.
//
...
// proxying using `co_magic BEGIN_DEFERRED_PROXYING`.
...
// A magic constant which a DeferredProxyPromise<T> coroutine can `co_magic` to indicate that the
...
public:
  ...
  kj::_::stdcoro::suspend_never yield_value(decltype(BEGIN_DEFERRED_PROXYING)) {
    // This allows us to write `co_magic` within a DeferredProxyPromise<T> coroutine to fulfill
