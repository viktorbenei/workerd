...
KJ_TEST("kj::Promise<DeferredProxy<T>>: `co_magic BEGIN_DEFERRED_PROXYING` fulfills outer promise") {
  kj::EventLoop loop;
  kj::WaitScope waitScope(loop);
  ...
  auto coro = [&]() -> kj::Promise<DeferredProxy<int>> {
    ...
    co_magic BEGIN_DEFERRED_PROXYING;
...
KJ_TEST("kj::Promise<DeferredProxy<T>>: unhandled exception before `co_magic BEGIN_DEFERRED_PROXYING`") {
  kj::EventLoop loop;
  kj::WaitScope waitScope(loop);
...
KJ_TEST("kj::Promise<DeferredProxy<T>>: unhandled exception after `co_magic BEGIN_DEFERRED_PROXYING`") {
  kj::EventLoop loop;
  kj::WaitScope waitScope(loop);
